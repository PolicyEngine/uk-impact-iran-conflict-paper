...
One structural point governs how the sweep should be read. The phase-in profile is built from each cap period's \emph{actual} forward-curve observation window, so a change in $\ell$ changes which observation windows contain the shock rather than sliding a fixed weight tuple along a calendar. A construction in which the cumulative weight carries no lag dependence returns an invariance that is an identity rather than a finding; this one does not, and the sweep asserts that its central row reproduces the headline exactly.
